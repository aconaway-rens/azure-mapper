...
```bash
git clone --depth 1 --branch v1.1.0 https://github.com/aconaway-rens/azure-mapper.git
cd azure-mapper
```
...
```bash
curl -L https://github.com/aconaway-rens/azure-mapper/archive/refs/tags/v1.1.0.tar.gz | tar xz
cd azure-mapper-1.1.0
```
...
current version — substitute it for `v1.1.0` above, or drop `--branch v1.1.0`
to track `main`.
