...
- **Load balancers** — an internal LB is drawn inside its frontend subnet,
  ...
  entirely. Both are linked to the members they balance; a backend NIC folded
  into a VM card is linked via that card, so the line always lands on something
  you can see.
...
```bash
git clone --depth 1 --branch v1.3.1 https://github.com/aconaway-rens/azure-mapper.git
cd azure-mapper
```
...
```bash
curl -L https://github.com/aconaway-rens/azure-mapper/archive/refs/tags/v1.3.1.tar.gz | tar xz
cd azure-mapper-1.3.1
```
...
current version — substitute it for `v1.3.1` above, or drop `--branch v1.3.1`
to track `main`.
...
```bash
git fetch --tags
git checkout v1.3.1          # whichever release you're moving to
